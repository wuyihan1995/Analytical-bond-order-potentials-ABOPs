...
/* -------------------------------  构造函数  --------------------------------------- */
PairTersoffZBLMy::PairTersoffZBLMy(LAMMPS *lmp) : PairTersoffMy(lmp)
{
  ...
  if (strcmp(update->unit_style,"metal") == 0) {
    ...
    global_e = 1.0;
  } else error->all(FLERR,"Pair tersoff/zbl/my requires metal or real units");
}
...
/* ----------------------------------------------------------------------
  ...
  --------------------------------------------------------------------- */
void PairTersoffZBLMy::read_file(char *file)
{
  int params_per_line = 21+1;                     //  修改，新增参数r0
  ...
  while (1) {
    ...
    params[nparams].ZBLcut = atof(words[20]);
    params[nparams].ZBLexpscale = atof(words[21]);
    ...
    if (params[nparams].c < 0.0 ||
		...
		params[nparams].r0 < 0.0)                //  修改，新增r0相关的判断条件
...
/* ----------------------------------------------------------------------
    对势作用中的排斥力项
  ----------------------------------------------------------------------  */
void PairTersoffZBLMy::repulsive(Param *param, double rsq, double &fforce,
                               int eflag, double &eng)
{
  ...
  double fforce_ters = param->biga * tmp_exp * (tmp_fc_d - tmp_fc*param->lam1); //  力表达式,无需修改
  double eng_ters = tmp_fc * param->biga * tmp_exp;                             //  能量表达式：Vr=fc*Aexp(-lam1*(r-r0))
...
/* ----------------------------------------------------------------------
  ...
  ---------------------------------------------------------------------  */
double PairTersoffZBLMy::ters_fa(double r, Param *param)
{
...
/* ----------------------------------------------------------------------
    计算Va对r_ij导数
  ---------------------------------------------------------------------  */
double PairTersoffZBLMy::ters_fa_d(double r, Param *param)
{
  if (r > param->bigr + param->bigd) return 0.0;
  //  修改，加入r0
...
double PairTersoffZBLMy::F_fermi(double r, Param *param)
{
...
double PairTersoffZBLMy::F_fermi_d(double r, Param *param)
{
