...
/* -*- c++ -*- ----------------------------------------------------------
   Modified Tersoff potential. Citation: Henriksson et al. J. Phys.: Condens. Matter 25 (2013) 445401 (13pp).
...
PairStyle(tersoff/my,PairTersoffMy)       //  设定Pair_style在LAMMPS中被引用时的名字，以及对应的源代码class名字

#else

#ifndef LMP_PAIR_TERSOFF_MY_H             //  修改，增加_MY_
#define LMP_PAIR_TERSOFF_MY_H

#include "pair.h"

namespace LAMMPS_NS {

class PairTersoffMy : public Pair {       //  修改，class名字与上面PairStyle函数中设定的一样
 public:
  PairTersoffMy(class LAMMPS *);  //  构造函数
  virtual ~PairTersoffMy();       //  析构函数
 ...
 protected:
  ...
  struct Param {
    //  Tersoff势函数参数
    double lam1,lam2,lam3;       //  exp函数中的系数
    double c,d,h;                //  g(theta)函数中的系数，h=cos(theta0_ijk)
    double gamma,powerm;         //  g(theta)函数中的gamma，exp函数中的指数m_ijk(=1)
    double powern,beta;          //  n_ij=1, beta_ij=1
    double biga,bigb,bigd,bigr;  //  即函数中的A, B, D, R
    double cut,cutsq;            //  截断半径设置的是 r_cut = R + D
    double c1,c2,c3,c4;          //  计算g(theta)所需的中间量
    int ielement,jelement,kelement;
    int powermint;               //  int(m_ijk)，保证输入的量是整数
  ...
  char **elements;              // names of unique elements  (不重复！！)
  int ***elem2param;            // mapping from element triplets to parameters
  int *map;                     // mapping from atom types to elements
  double cutmax;                // max cutoff for all elements
  int nelements;                // # of unique elements，即当前对应的参数集下标
  ...
  //  inlined functions for efficiency，设定内联函数
  //  计算g_ijk函数
